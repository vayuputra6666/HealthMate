...
export class MemStorage implements IStorage {
  ...
  // Weight tracking methods
  private weightEntries = new Map<number, WeightEntry>();
  private currentWeightId = 1;
  private userProfiles = new Map<number, UserProfile>();
  ...
  async createWeightEntry(weightEntry: InsertWeightEntry): Promise<WeightEntry> {
    ...
    const newEntry: WeightEntry = { ...weightEntry, id, createdAt: new Date() };
    this.weightEntries.set(id, newEntry);
    return newEntry;
  }

  async getWeightEntries(): Promise<WeightEntry[]> {
    return Array.from(this.weightEntries.values()).sort((a, b) => 
      new Date(b.date).getTime() - new Date(a.date).getTime()
    );
  }
  ...
  async createUserProfile(profile: InsertUserProfile): Promise<UserProfile> {
    ...
    const newProfile: UserProfile = { ...profile, id, createdAt: new Date(), updatedAt: new Date() };
  ...
  async getUserProfile(): Promise<UserProfile | undefined> {
    ...
    return profiles[0]; // For simplicity, return the first profile
  }
  ...
  calculateMaintenanceCalories(profile: UserProfile, weight: number): number {
    ...
    return Math.round(bmr * activityMultipliers.moderate); // Default to moderate
  }
