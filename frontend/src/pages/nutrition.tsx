...
export default function Nutrition() {
  ...
  const { data: meals, isLoading: mealsLoading, isError: mealsError } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/meals/${selectedDate}`);
      if (!res.ok) {
  ...
  const { data: recipes, isError: recipeError } = useQuery({
  ...
  const { data: calorieInfo, isError: calorieInfoError } = useQuery({
    queryKey: ["/api/maintenance-calories"],
    queryFn: async () => {
      const res = await fetch("/api/maintenance-calories");
      if (!res.ok) {
        throw new Error(`HTTP error! status: ${res.status}`);
      }
      return res.json();
  ...
  const dailyTotals = Array.isArray(meals) ? meals.reduce(
  ...
  if (mealsError || goalsError || recipeError || calorieInfoError) {
    return <EmptyState title="Error loading data" description="Failed to load nutrition data. Please try again later." />;
  }
